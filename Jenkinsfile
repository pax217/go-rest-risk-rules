@Library('ckt-libraries@1.0.3')
import static org.conekta.Github.*
import static org.conekta.SonarQube.*
import static org.conekta.Docker.*
...
node('sre-slave') {
    ...
    ansiColor('xterm') {
        ...
        final int SONAR_TIMEOUT_MINUTES = 8
        ...
        try {
            ...
            stage('Unit Test') {
                runUnitTest(this, repo_name, "go test ./... -coverprofile cover.out -tags musl", "/go/src/${repo_name}")
            }
            stage('Integration Test ') {
                runDockerComposeContainer(this, "docker-compose.ci.yml", BUILD_NUMBER , " httpserver", "make integration-tests")
            }
            stage('Acceptance Test ') {
                runDockerComposeContainer(this, "docker-compose.ci.yml", BUILD_NUMBER , " httpserver", "make acceptance-tests")
            }
